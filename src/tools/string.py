...
async def set(
    ...
    expiration: Optional[int] = None, host_id: Optional[str] = None,
) -> str:
    ...
    try:
        r: Redis = RedisConnectionManager.get_connection(host_id)
        if expiration:
...
@mcp.tool()
async def get(key: str, host_id: Optional[str] = None) -> Union[str, bytes]:
